    ...
    MemberSurvey.findByIdAndUpdate(id, updateQuery).exec()
        .then(updatedMemberSurvey => {
            ...
            if (updatedMemberSurvey) {
                ...
                MemberSurvey.find({
                    memberCollection: updatedMemberSurvey.memberCollection,
                    member: updatedMemberSurvey.member
                }).exec()
                    .then(foundMemberSurveys => {
                        log.info("Found " + foundMemberSurveys.length + " MemberSurveys for MemberCollection " + updatedMemberSurvey.memberCollection + " and Member " + updatedMemberSurvey.member);

                        // Calculate the completeness score of whole collection
                        let completedSurveys = foundMemberSurveys.filter(survey => survey.completeness === 100).length;
                        let totalSurveys = foundMemberSurveys.length;
                        let completenessScore = Math.round((completedSurveys / totalSurveys) * 100);

                        // Update the member collection with the new completeness score
                        MemberCollection.findByIdAndUpdate(updatedMemberSurvey.memberCollection, { completeness: completenessScore }).exec()
                            .then(() => {
                ...
                res.status(500).json({ message: "Error update request for MemberSurvey" });
            }
        })
        .catch(error => {
            log.error(error.message);
