...
//======== Controllers for when reports viewed through the history section =====
const historicNeighbour = async (memberSurveyList) => {
  ...
  let chapter1Results = memberSurveyList.find(
    (survey) =>
      survey.surveyTemplate.name ==
      "Community_Connections: [1] Help Request Tracker"
  );

  let chapter2Results = memberSurveyList.find(
    (survey) =>
      survey.surveyTemplate.name ==
      "Community_Connections: [2] My Story"
  );

  let chapter3Results = memberSurveyList.find(
    (survey) =>
      survey.surveyTemplate.name ==
      "Community_Connections: [3] Who's in My Circle"
  );

  let chapter4Results = memberSurveyList.find(
    (survey) =>
      survey.surveyTemplate.name ==
      "Community_Connections: [4] Activities and Interests"
  );

  let chapter5Results = memberSurveyList.find(
    (survey) =>
      survey.surveyTemplate.name ==
      "Community_Connections: [5] Goals"
  );

  let chapter6Results = memberSurveyList.find(
    (survey) =>
      survey.surveyTemplate.name ==
      "Community_Connections: [6] Health"
  );

  let chapter7Results = memberSurveyList.find(
    (survey) =>
      survey.surveyTemplate.name ==
      "Community_Connections: [7] Wellness"
  );

  let chapter8Results = memberSurveyList.find(
    (survey) =>
      survey.surveyTemplate.name ==
      "Community_Connections: [8] Action Tracker"
  );

  let chapter9Results = memberSurveyList.find(
    (survey) =>
      survey.surveyTemplate.name ==
      "Community_Connections: [9] Meeting Expectations - Your Experience"
  );

  let chapter10Results = memberSurveyList.find(
    (survey) =>
      survey.surveyTemplate.name ==
      "Community_Connections: [10] Post Survey: Health, Wellness and Social Outcomes Survey"
  );
...
const historicQofL = async (memberSurveyList) => {
...
// History section main controller calls the other functions based on collection
exports.Historic = async (req, res) => {
  ...
  try {
    ...
    if (!foundCollection) {
      ...
      // result based on type of collection
      ...
      } else if (req.params.collectionCode === 'qofl') {
        result = await historicQofL(foundCollection.memberSurveyList);
      }
